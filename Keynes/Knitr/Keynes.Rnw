...
plot(da2$Date, da2$GDP-da2$fitted.values[117:176], type = 'l', col = "Dark Green", main = "Gap between US GDP and its potenital", xlab = "Date", ylab = "USD bn")
@
\end{frame}
...
\begin{itemize}[<+-| alert@+>]
\item These are a series of data that change over time
\item There are \emph{Stationary} and \emph{Nonstationary} series
\item There may be \emph{Deterministic} and \emph{Stochastic} trends
\item Data may be \emph{Pro-cyclical} and \emph{Counter-cyclical}
\end{itemize}
\end{frame}
...
\end{document}
>>>>>>> 05b5801c9b096f2827e502e9e16c9ffb3e8414d8
\begin{itemize}[<+-| alert@+>]