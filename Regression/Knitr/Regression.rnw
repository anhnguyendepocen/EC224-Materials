...
\end{tabular}
\end{table}
The Adjusted $R^2$ is 0.08, therefore only 8\% of the BAC returns are explained by the returns of the market.  95\% confidence intervals for the $\beta$ are -0%.51 to 0.11. 
\end{frame}

\begin{frame}{Three additional ideas}
There are three thing to consider
\pause
\begin{itemize}[<+-| alert@+>]
\item How well do the data fit the model? $(R^2)$
\item How reliable is the estimated relationship?  Does $\beta_1 = 0$?
...
\end{itemize}
\pause
If we assume a normal distribution we can carry out hypotheses tests about coefficients like $\beta_1$
\end{itemize}
\end{frame}
