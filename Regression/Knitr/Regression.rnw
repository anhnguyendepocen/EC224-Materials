...
\item $\varepsilon_t$ are all the other factors that affect the inflation rate
\end{itemize}
\end{frame}
...
\item $\hat{\beta}$ is a random variable.  If OLS (\emph{Gauss-Markov} assumptions hold)
\begin{itemize}
...
\end{itemize}
\item If there are trends to the residuals 
\begin{itemize}
