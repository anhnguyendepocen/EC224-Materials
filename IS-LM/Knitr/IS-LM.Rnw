...
\end{itemize}
\pause
The slope of the PE curve may shift due to changes in (MPS, MPT, MPI)
\begin{itemize}[<+-| alert@+>]
...
\node at (2.9, 0.5) [above left] {\footnotesize MD};
...
\node at (0, 1.2) [left] {\footnotesize $i_i$};
%\node at (3, 1.5) [above left] {\footnotesize $i_2$};
%\node at (-0.1, 2) [above, rotate = 90] {Planned Expenditure};
%\node at (0, 0.4) [above, rotate = 90] {\small $A = 200$};
%\node at (2, 0) [below] {Income and output};
...
\item Demand for money will increase and raise interest rates, reducing affecting business investment and household spending
\item \text{Monetary policy is more effective}
...
\pause
\begin{itemize}[<+-| alert@+>]
\item \text{The central bank may \emph{accommodate} the fiscal expansion}
\item In this case, the fiscal policy will be more powerful
\item \text{The central bank may offset the fiscal expansion}
